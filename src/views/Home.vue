        ...
        <v-container>
            <v-card flat class="transparent mt-4 mb-16 pb-16">
                ...
                <v-card-actions class="px-4">
                    <v-btn :large="$vuetify.breakpoint.mdAndDown" :x-large="$vuetify.breakpoint.lgAndUp" @click="goto({ name: 'projects' })" color="primary">
                        <v-icon left>{{ $store.getters['navigation/mainMenuItems'][1].icon }}</v-icon>
                        See My Projects
                    </v-btn>
                    <v-btn :large="$vuetify.breakpoint.mdAndDown" :x-large="$vuetify.breakpoint.lgAndUp" @click="goto({ name: 'contact' })">
