<template>
    <v-bottom-navigation v-if="windowHeight > 512" app grow model-value="activeMenuItem.index" :height="48" fixed>
        ...
        >
            <v-icon :icon="menuItem.icon" color="dark" :style="{ opacity: (menuItem.name === activeMenuItem.item.name) ? 1 : 0.8 }"/>
            <span :class="{ 'text-dark': menuItem.name !== activeMenuItem.item.name }">{{ menuItem.text }}</span>
        </v-btn>
    </v-bottom-navigation>
</template>
