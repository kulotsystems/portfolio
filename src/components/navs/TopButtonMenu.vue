<template>
    <div v-if="windowWidth > 600">
        <v-btn
            ...
            :class="(menuItem.name === activeMenuItem ? ($store.getters.isLightMode ? 'primary--text ' : 'white--text ') : ($store.getters.isLightMode ? 'white--text ' : '')) + $store.getters['breakpoints/button/md']"
            :large="$vuetify.breakpoint.md || $vuetify.breakpoint.lg"
            :x-large="$vuetify.breakpoint.xl"
            :color="$store.getters.isLightMode ? 'white' : (menuItem.name === activeMenuItem ? 'primary' : 'grey')"
            elevation="0"
